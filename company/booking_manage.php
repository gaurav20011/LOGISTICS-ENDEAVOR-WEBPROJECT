                ...
                <div class="table-responsive">
                  <table class="table table-hover">
                    ...
                    <tbody>
                      ...
                      <!--table data starts -->
                      <?php $query = $admin->ret("SELECT * FROM ((`booking` INNER JOIN `user` ON booking.user_idb = user.user_id)INNER JOIN `service` ON booking.service_idb = service.service_id ) where `company_idb`=$s_variable  ORDER BY `booking_date`;");
                      while ($row = $query->fetch(PDO::FETCH_ASSOC)) { ?>
                        <tr>
                          ...
                          <?php if ($row['booking_status'] == 'booked') { ?>
                            <td><a type="button" href="controller/booking_controller.php?accept_booking=<?php echo $row['booking_id'] ?>" class="btn btn-primary btn-rounded btn-fw">Accept Booking</a></td>
                            <td><a type="button" class="btn btn-danger btn-rounded btn-fw" href="controller\booking_controller.php?delete_booking_id=<?php echo $row['booking_id'] ?>" onclick="return confirm('Are you sure you want to reject?');">Reject</a></td>
                          ...
                          <?php } elseif ($row['booking_status'] == 'booking accepted') { ?>
                            <td><a type="button" class="btn btn-primary btn-rounded btn-fw" href="controller\booking_controller.php?pickup_goods=<?php echo $row['booking_id'] ?>">Pickup Goods</a></td>
                          <?php } elseif ($row['booking_status'] == 'Goods picked') { ?>
                            <td><a type="button" class="btn btn-warning btn-rounded btn-fw" href="goods_location.php">Update current location</a></td>
                          <?php } elseif ($row['booking_status'] == 'reached') { ?>
                            <td><a type="button" class="btn btn-success btn-rounded btn-fw">Goods reached destination</a></td>
